...
const Home = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Last 3 transactions from all orders sorted by newest
        const recentOrders = [...orders]
          .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
          .slice(0, 3);

        setTransactions(recentOrders.map(o => ({
          id: o._id,
          name: o.name || "Customer",
          amount: `₹${(o.total || 0).toLocaleString('en-IN')}`,
          status: o.status === 0 ? "Pending" : o.status === 1 ? "Processing" : "Completed"
        })));

        // Chart: last 7 days revenue from completed orders
        const days = [];
        const revs = [];
        for (let i = 6; i >= 0; i--) {
          const d = new Date();
  ...
  const getStatusColor = (status) => {
    if (status === "Completed") return { color: "#059669", background: "#ecfdf5" };
    if (status === "Processing") return { color: "#d97706", background: "#fffbeb" };
              ...
              {transactions.length > 0 ? transactions.map((t) => (
                <tr key={t.id} style={{ borderBottom: "1px solid #f8fafc" }}>
                  ...
                  <td style={{ padding: "16px 12px" }}>
                    <span style={{ ...statusBadgeStyle, ...getStatusColor(t.status) }}>{t.status}</span>
                  </td>
                </tr>
              )) : (
...
const statusBadgeStyle = { padding: "5px 12px", borderRadius: "8px", fontSize: "0.75rem", fontWeight: "800", textTransform: "uppercase" };
